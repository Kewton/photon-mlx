...
RAG から独立した PHOTON runtime / training layer です。`PhotonModel`、`PhotonInference`、session state、checkpoint I/O、trainer を持ちます。`baseline_reporag` に依存してはいけません。

OSS MVP では Multi-turn RAG が主プロダクトですが、`photon_mlx` は experimental standalone API として公開します。PHOTON-only の利用者は `photon-train` / `photon-generate` または `photon_mlx` の public API を使います。

### `baseline_reporag/photon_pipeline.py`

PHOTON と RAG の統合層です。retrieval、evidence pack、session carryover、citation 制御など RAG 固有の処理に PHOTON score を組み込みます。PHOTON 本体へ RAG 依存を逆流させないため、この層を境界として扱います。
