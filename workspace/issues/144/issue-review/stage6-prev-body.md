...
pip install -e .  # photon-mlx 本体
pip install "transformers<5.0" "sentence-transformers<5.0"
```
...
- [ ] Task 0 (前提): `requirements.txt` に `huggingface_hub` を明示追加 (Task 1 が直接 import するため)
...
- [ ] V2 が V4 を上回った場合 (低確率): #137 reopen + 新 PR で `configs/institutional_docs.yaml` 採用 variant を更新 (採用判定再評価)
