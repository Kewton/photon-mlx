...
- `photon_mlx/trainer.py` の **既存 `resume_from` / cosine schedule ロジックをそのまま利用** する。本 Issue では trainer.py 本体のロジック改修は不要 (新フィールド `train_corpora_mix` への分岐追加のみで、`load_checkpoint` / `_build_lr_schedule` / `train` の既存 path は変更しない)。意図せざる変更で既存 168 tests が破綻しないようガードする。
...
- **config 更新 (S3-007 反映: 後方互換)**: 既存 `configs/institutional_docs_photon.yaml` (#112 / #126 baseline 再現用) は **直接編集せず維持** し、本 Issue 用には **新規 `configs/institutional_docs_photon_retrain.yaml` を作成** する。新 yaml で `model.provider` (photon) と `paths.checkpoint_root` 配下の新パス (例: `./checkpoints/photon_institutional_15000_20260428/`)、および新 hyperparam (`lr=3e-5` / `warmup_ratio=0.0` / `max_steps=10000-20000` / `train_corpora_mix`) を持つ。これにより #112 / #126 既存 eval の再現性を保つ。
...
- `photon_mlx/trainer.py` (**既存 `resume_from` / `_build_lr_schedule` / `train` のロジックは改修不要**。新フィールド `train_corpora_mix` への分岐追加のみ。S3-004 反映で誤改修ガード)
- `photon_mlx/data.py` (**新関数 `iterate_mixed_batches(corpus_paths: dict[str, float], ...)` を追加し、既存 `iterate_batches()` の signature は変更しない**。S3-001 反映で後方互換固定)
- `photon_mlx/tests/test_training.py` (**新規 test 追加**: (a) `test_resume_from_continual_learning` - resume_from + warmup_ratio=0.0 + cosine reset で step counter が初期値+1 / 既存重みが恒等でないこと / optimizer state が新規 / LR が cosine 起点 (warmup スキップ) から開始することを assert (S3-002 反映)、(b) `iterate_mixed_batches` 単体テスト)
- `tests/test_generate_institutional_training_corpus.py` (新規、eval リーク検出時に exit code 非ゼロになる pytest を含む、S3-009 反映)
- `torch_ref/config.py` (`TrainingConfig` に `train_corpora_mix: dict[str, float] | None` 追加)
- `scripts/generate_institutional_training_corpus.py` (新規、訓練 corpus 生成、eval リーク検証内蔵)
...
- [ ] **`photon_mlx/tests/test_training.py` に resume_from + cosine schedule reset の unit test を追加** (`test_resume_from_continual_learning`: warmup から始まらず continual の min_lr→max_lr 起点になること / step counter / optimizer state 新規 / 既存重み非恒等 を検証、S3-002 反映)
...
| **`resume_from` が想定外の LR 起点を取り 600 step 重みを破壊** (S3-002 反映) | 既存 mulmoclaude 重み喪失 = scratch 同等に劣化 | `photon_mlx/tests/test_training.py` に `test_resume_from_continual_learning` を**学習開始前**に追加し、warmup_ratio=0.0 時に cosine が peak ではなく min_lr→max_lr 起点を取ることを CI で恒常確認 |
