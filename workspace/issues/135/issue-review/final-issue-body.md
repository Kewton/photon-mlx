...
- **制度文書 JP 50%+ 追加**: institutional_documents 4228 md から学習用 turn 系列を生成
  ...
  - **photon_mlx/data.py 拡張 (S3-001 / S5-005 反映: 後方互換固定 / DR1-005 反映: val_split 簡素化)**: **新関数 `iterate_mixed_batches(corpus_paths: dict[str, float], context_length, batch_size, ...)` を追加し、既存 `iterate_batches()` の signature は変更しない**。これにより既存 4 callsite (`photon_mlx/trainer.py:297-303` の 2 箇所、`scripts/train_photon_quick.py:48-49` の 2 箇所) と `photon_mlx/tests/test_training.py:23` 系既存 unit test (`test_pack_sequences` / `test_create_batches` / `test_load_jsonl` 等 27+ 件) は無改修で動作する。`torch_ref/config.py` の `TrainingConfig` に新フィールド `train_corpora_mix: dict[str, float] | None` と `val_split: float = 0.05` を追加し (DR1-005: `val_corpora_mix` dict 化は廃止し、train pool 内 sequence-level split に簡素化)、`photon_mlx/trainer.py` 側は `train_corpora_mix` が `None` のときは従来通り `iterate_batches` を呼び、`dict` のとき `iterate_mixed_batches` を呼ぶ分岐とする。
    ...
    - validation は **`val_split: float = 0.05` (DR1-005)** が指定されていれば train pool 内から sequence-level split で val を切り出し (train_corpora_mix と同じ JP/EN 50/50 比率を val でも維持)、`val_split=0.0` (未指定) なら従来 `val_corpus` の単一 corpus を使う。`val_corpora_mix` dict は導入しない (YAGNI)。
...
- `photon_mlx/trainer.py` の **既存 `resume_from` / cosine schedule ロジックをそのまま利用** する。本 Issue では trainer.py 本体の schedule / checkpoint ロジック改修は不要 (新フィールド `train_corpora_mix` / `val_split` への分岐追加のみで、`load_checkpoint` / `_build_lr_schedule` / `train` の既存 path は変更しない、DR1-005)。意図せざる変更で既存 168 tests が破綻しないようガードする。
...
- **checkpoint path 粒度 (S7-002 反映)**:
  - training output root = `checkpoints/photon_institutional_retrain_<yyyymmdd>/`
  - eval/adopt candidates = `checkpoints/photon_institutional_retrain_<yyyymmdd>/step_010000`, `.../step_015000`, `.../step_020000`
  - `weights.npz` / `state.json` を持つ **leaf directory** を runtime / eval / report の `checkpoint_path` として扱う
  - 採用後に leaf contents を `checkpoints/photon_institutional_<step>_<yyyymmdd>/` へ昇格コピーする場合は、copy 後 directory が直接 `weights.npz` / `state.json` を含むことを確認する
...
- **checkpoint ディレクトリ命名規則**: 学習中は `checkpoints/photon_institutional_retrain_<yyyymmdd>/step_<cumulative_step>/` を使用し、採用確定後のみ必要に応じて `checkpoints/photon_institutional_<step>_<yyyymmdd>/` (例: `checkpoints/photon_institutional_15000_20260428/`) へ leaf contents を昇格コピーする。いずれの場合も `configs` / reports / runtime には `weights.npz` / `state.json` を直接含む leaf path を書く (S7-002 反映)。
- **保管先判定**:
...
- **config 更新 (S3-007 / S5-005 / S7-001 / S7-002 / DR1-005 反映: 後方互換)**: 既存 `configs/institutional_docs_photon.yaml` (#112 / #126 baseline 再現用) は **直接編集せず維持** し、本 Issue 用には **新規 `configs/institutional_docs_photon_retrain.yaml` を作成** する。新 yaml で `model.provider: "photon"`、`model.checkpoint_path: ./checkpoints/photon_institutional_retrain_<yyyymmdd>/step_015000` のような **leaf checkpoint path**、および新 hyperparam (`lr=3e-5` / `warmup_ratio=0.0` / **`min_lr=3e-6`** / `max_steps=10000-20000` / `train_corpora_mix` (sum=1.0 strict) / `val_split=0.05`) を持つ (`min_lr=3e-6` は `_build_lr_schedule` の cosine_decay 経路を有効化する必須キー条件、`val_split` は DR1-005 で `val_corpora_mix` を廃止して train pool 内 sequence-level split に簡素化)。これにより #112 / #126 既存 eval の再現性を保つ。
- **PHOTON runtime checkpoint load (S7-001 / DR1-002 反映)**: `baseline_reporag/photon_pipeline.py::_build_photon_deps()` は `model.checkpoint_path` が設定されている場合、`PhotonModel(photon_cfg)` 構築直後に `photon_mlx.checkpoint.load_checkpoint(model, checkpoint_path)` を呼ぶ。**`load_checkpoint` は新規 `photon_mlx/checkpoint.py` (inference 専用 module) に物理移管し、trainer.py は `from .checkpoint import load_checkpoint` で再 export する。`_build_photon_deps` は `photon_mlx.checkpoint` のみを import し、`photon_mlx.trainer` への直接 import は禁止 (DR1-002 境界規約)**。checkpoint path が missing / corrupt の場合は silent fallback せず eval を fail させる。これにより `scripts/run_multi_turn_eval.py` + `build_pipeline(cfg)` 経由でも再学習済み weights を評価する。
...
- **比較表出力 (S7-002 反映)**: `reports/institutional_photon_mt_eval_v2.md` に **旧 PHOTON 値・新 PHOTON 値・baseline 値** を 1 表で並べる。新 PHOTON 行には `checkpoint_path`, `cumulative_step`, `resume_base_step=600`, `additional_updates` を列として含める。
...
- **採用 checkpoint 確定後のドキュメント更新 (S3-005 反映)**:
  ...
  - `workspace/mvp/roadmap.md` Phase 2 conditional 表 (>6% 分岐) に「実行済」マーキング、必要に応じて 3-6% fallback 行も追記 (DR2-011)
...
- `photon_mlx/trainer.py` (**既存 `resume_from` / `_build_lr_schedule` / `train` の schedule / checkpoint ロジックは改修不要**。新フィールド `train_corpora_mix` / `val_split` への分岐追加のみ。`load_checkpoint` は新規 `photon_mlx/checkpoint.py` へ物理移管し、trainer.py は `from .checkpoint import load_checkpoint` で再 export (DR1-002)。S3-004 / S5-001 / S5-002 / S5-005 / S7-002 / DR1-002 / DR1-005 反映で誤改修ガード)
- `baseline_reporag/photon_pipeline.py` (`_build_photon_deps()` で `model.checkpoint_path` を読み、`photon_mlx.checkpoint.load_checkpoint()` を呼ぶ。`photon_mlx.trainer` への直接 import は禁止 (DR1-002)。missing/corrupt checkpoint は fail、S7-001 / DR1-002 反映)
...
- `torch_ref/config.py` (`TrainingConfig` に `train_corpora_mix: dict[str, float] | None` / `val_split: float = 0.05` 追加 (DR1-005)。空 dict / 負 weight / 全 weight 0 / 非数値 weight / sum != 1.0 (±1e-6) を拒否する strict validation を追加、S7-003 / DR1-003 / DR1-005 反映)
- `torch_ref/tests/test_model.py` (`TrainingConfig` defaults / YAML load / invalid mix ratio / 既存 config 後方互換の regression test 追加、S7-003 反映)
...
- `baseline_reporag/tests/` (photon provider smoke test 新規追加。`photon_mlx.checkpoint.load_checkpoint` が `model.checkpoint_path` で呼ばれること、`photon_mlx.trainer` への直接 import が無いこと、missing/corrupt checkpoint が fail することを assert、S3-006 / S7-001 / DR1-002 反映)
- `.github/workflows/*.yml` (CI 設定確認 / 必要時 LFS 有効化、S3-006 反映)
- `.github/workflows/tests.yml` (CI で pytest を必須にする場合は新規追加。少なくとも `python -m pytest --ignore=data/training/` を実行、S7-004 反映)
- `reports/institutional_photon_mt_eval_v2.md` (新規、旧 PHOTON / 新 PHOTON / baseline 比較表。`checkpoint_path`, `cumulative_step`, `resume_base_step`, `additional_updates` を記録、S7-002 反映。**旧 `reports/institutional_photon_mt_eval.md` (#113) は履歴として保持、_v2.md は冒頭で旧 report への cross-link を持ち、両 report は並列保持** = `gate2_judgment_v4_final.md` → v5 と同一パターン、DR2-007)
...
- [ ] **`torch_ref/tests/test_model.py` に mix config regression を追加**: defaults が `train_corpora_mix=None` / `val_split=0.05`、YAML の `train_corpora_mix` と `val_split` が保持される、不正 ratio (空 dict / 負 weight / 非数値 / sum != 1.0±1e-6) が reject される、既存 config load が後方互換で通る (S7-003 / DR1-003 / DR1-005 反映)
...
- [ ] `reports/institutional_photon_mt_eval_v2.md` で再測定値を記録 (旧 PHOTON / 新 PHOTON / baseline 比較表。新 PHOTON は `checkpoint_path`, `cumulative_step`, `resume_base_step=600`, `additional_updates` を明記、S7-002 反映)
...
- [ ] **PHOTON runtime が採用 checkpoint を実際に load すること**: `baseline_reporag/photon_pipeline.py::_build_photon_deps()` が `model.checkpoint_path` の leaf directory を `photon_mlx.checkpoint.load_checkpoint()` で読む (`photon_mlx.trainer` への直接 import は禁止、DR1-002)。`baseline_reporag/tests/` の smoke test で `photon_mlx.checkpoint.load_checkpoint` 呼び出しと missing/corrupt checkpoint fail を検証 (S7-001 / DR1-002 反映)
- [ ] **`pytest` 全 tests (約 507/509) が新 corpus / 新 checkpoint 取得手順下で破綻しないこと**。CI 必須にする場合は `.github/workflows/tests.yml` 等で `python -m pytest --ignore=data/training/` を実行。CI にしない場合はローカル M3 Ultra で同 command を実行しログを report に添付 (S3-006 / S7-004 反映)
- [ ] **`baseline_reporag/tests/` に photon provider smoke test 新規追加** (`pipeline_factory.create_query_pipeline` photon 経路をモック checkpoint で起動し、checkpoint load まで確認、S3-006 / S7-001 反映)
...
- **ETA 再見積もり (S7-006 反映)**: Day 3 の 1K step lr-finder で sec/step を実測し、累計 10K/15K/20K までの ETA を再計算してから本走行に入る。ETA が 5 日を超える場合は `grad_accum=4` / 10K early stop / #137 完了後の夜間実行へ切替を検討する
...
| **65,536 tokens/step による GPU 占有長期化** (S7-006 反映) | M3 Ultra / self-hosted runner を想定以上に占有、#137 / weekly eval と競合 | Day 3 の 1K step lr-finder で sec/step を測定し ETA を再計算。5 日超なら `grad_accum=4` / 10K early stop / 夜間実行へ切替 |
...
| **#137 との GPU / index / config 競合** (S7-005 反映) | #135 学習結果が #137 採用 variant 前提とずれる、M3 Ultra GPU/HF cache/index lock が競合 | #137 が open の間は #137 を先行完了し、採用/非採用結果を `configs/institutional_docs.yaml` と reports に反映してから #135 Day 3 学習へ進む。#135 と #137 の実機 eval/training は同時実行しない |
...
### #137 との実行順序 (S7-005 / S7-006 反映)
- #137 (institutional 多言語 embedding/reranker 5-variant 実機 A/B) が open の間は、**#137 を先行完了 → 採用/非採用結果を `configs/institutional_docs.yaml` / reports に反映 → #135 Day 3 学習開始** の順序を推奨する。
- #137 は `SentenceTransformer` / `CrossEncoder` と HF cache、`data/indexes/institutional_documents*`、M3 Ultra GPU/MPS を使用するため、#135 の MLX 学習/eval と同時実行しない。
- #135 本走行中は self-hosted GitHub Actions runner / weekly eval の占有にも注意し、Day 3 lr-finder の sec/step から runner 不在時間を見積もってから 10K-20K 本走行へ入る。
