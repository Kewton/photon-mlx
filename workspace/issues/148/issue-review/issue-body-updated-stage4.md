...
subtask:
1. `baseline_reporag/photon_pipeline.py:_build_photon_deps` の実装を確認し、`checkpoint_path` を読んで `trainer.load_checkpoint(model, path)` (または `photon_mlx/inference.py:load_inference_model`) を呼ぶ経路が存在するか確認する。
...
3. `photon_mlx/inference.py:137` のコメント「Check model.checkpoint_path and load result」が実体を伴っているか確認する。
...
> **S3-008 実行環境前提**: Phase B は **Mac Studio M3 Ultra (>=128GB unified memory) を必須要件** とする。Gemma4 26B MoE は mlx_lm.load 時に weight 展開で peak memory が 64GB マシンで OOM を引き起こす可能性がある。Phase B 開始前に `mlx_lm.load(gemma-4-26b-a4b-4bit)` で peak RSS を測定し、使用可能な最大メモリを確認すること。Mac mini (~64GB) では failure する可能性があるため、実機 spec を確認してから着手すること。
...
- `photon_mlx/inference.py`: line 137 コメント「Check model.checkpoint_path and load result」の実装確認・修正
...
- [ ] `photon_mlx/inference.py:137` が load を実際に実行することを確認
...
- [ ] **前提**: Phase B 実行環境が Mac Studio M3 Ultra (>=128GB unified memory) であることを確認。`mlx_lm.load(gemma-4-26b-a4b-4bit)` で peak RSS 測定 (VSZ ≤ <X>GB を事前試算)。
...
- [ ] Phase C 影響ファイル節に列挙した文書 6 件 (docs/deployment.md, troubleshooting.md, tutorial.md, workspace/mvp/*.md, README.md) の memory footprint / model_id を更新
