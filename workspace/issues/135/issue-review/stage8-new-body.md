...
- **config 更新 (S3-007 / S5-005 / S7-001 / S7-002 反映: 後方互換)**: 既存 `configs/institutional_docs_photon.yaml` (#112 / #126 baseline 再現用) は **直接編集せず維持** し、本 Issue 用には **新規 `configs/institutional_docs_photon_retrain.yaml` を作成** する。新 yaml で `model.provider: "photon"`、`model.checkpoint_path: ./checkpoints/photon_institutional_retrain_<yyyymmdd>/step_015000` のような **leaf checkpoint path**、および新 hyperparam (`lr=3e-5` / `warmup_ratio=0.0` / `max_steps=10000-20000` / `train_corpora_mix` / `val_corpora_mix`) を持つ。これにより #112 / #126 既存 eval の再現性を保つ。
- **PHOTON runtime checkpoint load (S7-001 反映)**: `baseline_reporag/photon_pipeline.py::_build_photon_deps()` は `model.checkpoint_path` が設定されている場合、`PhotonModel(photon_cfg)` 構築直後に `photon_mlx.trainer.load_checkpoint(model, checkpoint_path)` を呼ぶ。checkpoint path が missing / corrupt の場合は silent fallback せず eval を fail させる。これにより `scripts/run_multi_turn_eval.py` + `build_pipeline(cfg)` 経由でも再学習済み weights を評価する。
...
- `photon_mlx/trainer.py` (**既存 `resume_from` / `_build_lr_schedule` / `train` の schedule / checkpoint ロジックは改修不要**。新フィールド `train_corpora_mix` / `val_corpora_mix` への分岐追加のみ。S3-004 / S5-001 / S5-002 / S5-005 / S7-002 反映で誤改修ガード)
- `baseline_reporag/photon_pipeline.py` (`_build_photon_deps()` で `model.checkpoint_path` を読み、`load_checkpoint()` を呼ぶ。missing/corrupt checkpoint は fail、S7-001 反映)
...
- `torch_ref/config.py` (`TrainingConfig` に `train_corpora_mix: dict[str, float] | None` / `val_corpora_mix: dict[str, float] | None` 追加。空 dict / 負 weight / 全 weight 0 / 非数値 weight を拒否する validation を追加、S7-003 反映)
...
- `baseline_reporag/tests/` (photon provider smoke test 新規追加。`load_checkpoint` が `model.checkpoint_path` で呼ばれること、missing/corrupt checkpoint が fail することを assert、S3-006 / S7-001 反映)
...
- `reports/institutional_photon_mt_eval_v2.md` (新規、旧 PHOTON / 新 PHOTON / baseline 比較表。`checkpoint_path`, `cumulative_step`, `resume_base_step`, `additional_updates` を記録、S7-002 反映)
...
- [ ] **`torch_ref/tests/test_model.py` に mix config regression を追加**: defaults が `None`、YAML の `train_corpora_mix` / `val_corpora_mix` が保持される、不正 ratio が reject される、既存 config load が後方互換で通る (S7-003 反映)
...
- [ ] **PHOTON runtime が採用 checkpoint を実際に load すること**: `baseline_reporag/photon_pipeline.py::_build_photon_deps()` が `model.checkpoint_path` の leaf directory を `load_checkpoint()` で読む。`baseline_reporag/tests/` の smoke test で `load_checkpoint` 呼び出しと missing/corrupt checkpoint fail を検証 (S7-001 反映)
