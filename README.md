...
## Multi-turn RAG の課題と PHOTON-RepoRAG のアプローチ

Multi-turn RAG では、ユーザーの質問が省略・照応・非自立質問になりやすく、会話履歴を踏まえた query rewriting / query contextualization がしばしば使われます。一方で、retriever / reranker は多くの場合 query-document relevance を中心に候補を選ぶため、generator が回答に必要とする十分・適切な evidence selection とはミスマッチが生じることがあります。

その結果、会話履歴の解釈と evidence selection が分離されたパイプラインでは、過去会話・現在質問・取得 evidence の接続が不安定になりやすい、という課題があります。

PHOTON-RepoRAG は、この課題に対して PHOTON の階層的 working memory を活用します。PHOTON は、文脈を token / mid / top 相当の複数粒度の latent state として扱うため、細かい語句レベルの関連性と、会話全体の粗い文脈を同じ scoring process の中で考慮できます。

この OSS では、現在ターン由来の token / mid-level 表現と、会話履歴から集約した session-level の coarse state を用いて、候補 evidence chunk との per-level similarity を計算します。それらを重み付き統合した `PHOTON score` により、現在質問に直接関係する evidence と、過去会話から補うべき evidence を ranking / pruning の中で考慮します。

これにより、LLM に会話履歴や検索結果をそのまま大量に渡すのではなく、回答に必要な文脈と根拠を選別して渡すことを目指します。

PHOTON は検索エンジンや回答生成器を置き換えるものではありません。PHOTON-RepoRAG では、PHOTON を Multi-turn RAG の前段で、会話文脈と evidence candidates を接続する判断レイヤーとして活用しています。技術詳細は [PHOTON Technical Overview](docs/photon_technical_overview.md) を参照してください。
...
このリポジトリの主プロダクトは Multi-turn RAG です。`photon_mlx/` は RAG から独立した PHOTON runtime / training layer としても利用できますが、OSS MVP では experimental standalone API という位置づけです。PHOTON 単体利用の導線は [PHOTON standalone guide](docs/photon_standalone.md) を参照してください。
...
>
> **MVP v0.1.0 の公開形態**: まず GitHub Release と source checkout を主導線にします。Streamlit 管理アプリは repository checkout から起動してください。wheel / sdist は release artifact として検証済みですが、初回 MVP では checkpoint や外部モデル重みは同梱しません。
...
# MVP v0.1.0 では repository checkout から editable install する。
# wheel artifact を使う場合も、Streamlit 管理アプリは source checkout から起動する。
pip install -e .
...
#### 5. 最小デモ corpus で baseline 動作確認する場合

実データや PHOTON checkpoint を用意する前に、同梱の小さな markdown corpus で baseline の ingest / index / ask 導線を確認できます。
...
`ask` は選択した YAML に設定された回答生成モデルを使います。embedding / generation model が未取得の場合、初回実行時に外部モデルの取得が発生することがあります。詳細は [`examples/minimal_corpus/README.md`](examples/minimal_corpus/README.md) を参照してください。
...
- [PHOTON standalone guide](docs/photon_standalone.md): PHOTON 単体 runtime / training API の使い方
