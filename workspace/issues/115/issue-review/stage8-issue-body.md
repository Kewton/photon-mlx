...
`_DOMAIN_TEMPLATES["institutional_docs"]` の merge 内容:
- source of truth は **実装時点の `configs/institutional_docs.yaml` と `tests/test_pipeline_factory_yaml_invariants.py` の institutional pin 値** とする。#137 等の institutional retrieval A/B 採用 PR が先に入った場合、古い値を hard-code して巻き戻さない。
- pre-#137 の現行値で実装する場合の merge 内容 (3 キー):
  - `indexing.embedding.model_id`: 多言語 e5-small (`intfloat/multilingual-e5-small`)
  - `indexing.symbol_graph.enabled`: false
  - `retrieval.reranker.model_id`: **当面 `cross-encoder/ms-marco-MiniLM-L-6-v2` 据え置き**。bge-reranker-v2-m3 等の多言語 reranker への切り替えは #133/#137 (institutional retrieval A/B) 完了後に follow-up Issue で対応する。
- #137 (institutional retrieval A/B) が先に merge 済みの場合の merge 内容 (5 キー):
...
- 既存 `BEST_PRACTICE_KEYS` (5 キー) と `_DOMAIN_TEMPLATES` は責務が異なるためそのまま共存させる。
- `_DOMAIN_TEMPLATES` の merge 対象は **base_profile が `institutional_docs` の時のみ effect する** 仕様に絞る (S3-002 方針 (b) 採用)。photon_small / photon_tiny / photon_long_context 等の base に institutional_docs template をかぶせるケースは **不可** とし、warning 表示または単純に no-op とする。
  - 理由: photon_small は `configs/photon_small.yaml:53-57` で `sentence-transformers/all-MiniLM-L6-v2` (英語、384 次元) を使用しており、wizard で multilingual-e5-small や bge-m3 に切り替わると既存 `data/indexes/<repo_id>/embedding/` 配下の index と embedding model_id 不整合 (model_id.txt persist 値と cfg 値の乖離) を起こすため。
  - 補足: `institutional_docs.yaml` が既に該当キーの正解値を持つため実質 no-op に近いが、将来 `institutional_docs.yaml` の値が乖離した場合の整合性担保ロジックとして wizard 側にも保持する。T-C6 では `base_yaml_text` に意図的に異なる値を入れ、`_DOMAIN_TEMPLATES` によって source of truth の正解値へ上書きされることを検証する。
...
- 文字数 0 または **非空白文字数 0** のときは `other` を返す (ZeroDivisionError 回避)。空白のみ入力の期待値も `other` とする。
...
- 注入方式: `build_messages()` 内部で `detect_language(question)` を呼び出し、`_SYSTEM` を分岐する (S3-004 方針 (b) 採用)。
  ...
  - `baseline_reporag/photon_pipeline.py:1005-1010` の callsite を含む 2 callsite (`baseline_reporag/pipeline.py:201-205` と `baseline_reporag/photon_pipeline.py:1005-1010`) + `baseline_reporag/tests/test_photon_pipeline.py:368-393` の `TestBuildMessagesSessionSummary` test class は変更不要。
...
- 単体テスト (`baseline_reporag/tests/test_prompt.py`, `baseline_reporag/tests/test_photon_pipeline.py`, `tests/test_photon_app_components.py`)
- 再照合対象 (`tests/test_pipeline_factory_yaml_invariants.py` の institutional pin 値)
...
- [ ] `_DOMAIN_TEMPLATES["institutional_docs"]` の値が実装時点の `configs/institutional_docs.yaml` と `tests/test_pipeline_factory_yaml_invariants.py` の institutional pin 値と一致する (#137 が先に merge 済みなら bge-m3 / bge-reranker-v2-m3 / batch_size=32 / max_input_chars=8192 を反映)
...
- [ ] **空白のみ入力では `detect_language()` が `other` を返し、`en` 判定の非空白文字数 0 で ZeroDivisionError を起こさない**
- [ ] **英語質問入力時に `build_messages()` の output が現状と完全一致 (golden snapshot test) すること**
- [ ] **日本語の非制度文書質問入力時に、system prompt が条文番号引用を無条件に強制しないこと**
- [ ] **PHOTON follow-up 相当の `build_messages(question=<日本語>, include_few_shot=False, session_summary=...)` がシグネチャ変更なしで動き、条件付き日本語ヒントが system prompt に残ること**
...
- [ ] **`tests/test_photon_app_components.py` の T-C6 (`TestGenerateYamlFromWizard`) に `test_institutional_docs_template_merges_3_keys` もしくは #137 merge 後は採用キー数に合わせた同等 test を新規追加し、`base_profile="institutional_docs"` かつ `base_yaml_text` に意図的に異なる値を入れても、生成 YAML が source of truth の値へ上書きされることを assert する**:
  - pre-#137: `indexing.embedding.model_id == intfloat/multilingual-e5-small`, `retrieval.reranker.model_id == cross-encoder/ms-marco-MiniLM-L-6-v2`, `indexing.symbol_graph.enabled == false`
  - post-#137: `indexing.embedding.model_id == BAAI/bge-m3`, `indexing.embedding.batch_size == 32`, `indexing.embedding.max_input_chars == 8192`, `retrieval.reranker.model_id == BAAI/bge-reranker-v2-m3`, `indexing.symbol_graph.enabled == false`
...
- 依存: #112（config 確立）
- 関連: #133 / #137 (institutional retrieval A/B) — reranker/embedding 採用判定はここで決定。本 Issue は採用前提を取らず据え置き。ただし #137 が先に merge 済みの場合は、採用済み institutional pin 値を `_DOMAIN_TEMPLATES` に反映し、旧値へ巻き戻さない。
- **#133/#137 同期メンテ**: `embedding.model_id` / `reranker.model_id` / bge-m3 採用時の `embedding.batch_size` / `embedding.max_input_chars` は institutional retrieval A/B 採用後に follow-up Issue で同時更新する。本 Issue の wizard merge 値は実装時点の `configs/institutional_docs.yaml` と `tests/test_pipeline_factory_yaml_invariants.py` の pin 値を反映する。`INSTITUTIONAL_EMBEDDING_MODEL_ID` / `INSTITUTIONAL_RERANKER_MODEL_ID` / `INSTITUTIONAL_EMBEDDING_MAX_INPUT_CHARS` pin 確定時に `_DOMAIN_TEMPLATES` も同時更新が必要。

## 並列開発

- **D-3 (prompt 調整)** は他 Issue と完全独立で先行可能。
- **D-2 (wizard template)** は #133/#137 reranker/embedding 採用前提で先行する場合は `retrieval.reranker.model_id` を据え置き (`cross-encoder/ms-marco-MiniLM-L-6-v2`) とし、#133/#137 完了後に follow-up Issue で更新する。
- #137 が #115 より先に merge された状態で #115 を実装・rebase する場合は、pre-#137 の 3 キーではなく post-#137 の採用キーへ `_DOMAIN_TEMPLATES` と T-C6 の期待値を更新する。
- 本 Issue 内で D-2 / D-3 を 1 PR に同梱しても、#133/#137 完了を待つ必要はない (reranker 値据え置きのため)。ただし merge 前に `configs/institutional_docs.yaml` と invariant pin の最新値を再確認する。
...
### Stage 7 (影響範囲レビュー 2回目, 2026-04-27)
対応した指摘:
- **S7-001** (Must Fix): #137 merge-order guard を追加 — 実装時点の institutional pin 値を source of truth とし、#137 が先に入った場合は bge-m3 / bge-reranker-v2-m3 / batch_size=32 / max_input_chars=8192 を `_DOMAIN_TEMPLATES` と T-C6 期待値へ反映する方針に修正
- **S7-002** (Should Fix): `detect_language()` の非空白文字数 0 条件を `other` と明記し、空白のみ入力で ZeroDivisionError を避ける受入条件を追加
- **S7-003** (Should Fix): PHOTON follow-up (`include_few_shot=False`, `session_summary` あり) 経路の影響ファイルと受入条件を追加
