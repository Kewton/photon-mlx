...
本 Issue の採用方針 B では、実 `PhotonModel` + test 内で生成した実 checkpoint + production load path (`_build_photon_deps` → `_load_photon_checkpoint` → `photon_mlx.trainer.load_checkpoint`) を通る integration test を 1 件以上追加し、「random-init weight が production で silently 動いていた」型の構造的事故を CI で検出可能にする。実 tokenizer / 実 retrieval / `pipeline.query(...)` まで含む full E2E は候補方針 A として scope 外 follow-up に分離する。
...
def test_photon_pipeline_with_self_trained_minimal_ckpt(tmp_path):
    """最小 PhotonConfig で 1 step 学習 → save → production load path → PhotonInference smoke が走る"""
    # 1. 最小 config で PhotonModel を構築
    #    既存 photon_mlx/tests の _tiny_cfg に寄せる:
    #    base_embed_dim=16, hidden_size=64, intermediate_size=128,
    #    num_heads=4, head_dim=16, tokenizer.vocab_size=256,
    #    chunk_sizes=[4, 4], encoder/decoder_layers_per_level=[1, 1]
    ...
    # 4. (i) の場合は `deps['photon_inference']` 経由で smoke を流す
    #    (例: generate_answer(..., max_new_tokens=1) または hierarchical_prefill)。
    #    これは full retrieval/query E2E ではなく checkpoint load path integration として扱う。
...
- [ ] テスト構造の最低保証 (mock を使わない範囲を明示):
   ...
   - [ ] tokenizer は HF download を避けるため `_load_hf_tokenizer` を local fixture に置き換えてよい (test isolation のため許容)。#138 tokenizer mismatch invariant と衝突しないよう、fixture の `vocab_size` は `_load_hf_tokenizer(tokenizer_id, expected_vocab_size)` の `expected_vocab_size` と一致させる
   ...
   - [ ] integration test は他 test と独立して走らせるため `tests/integration/` 配下に分離 (既に scope 内で確定済)。本 Issue では pytest-xdist / pytest-forked 依存を追加しないため、CI では `python -m pytest tests/integration/test_photon_real_weights.py -v` のように integration test を単独・シリアル実行する。`--forked` / `-n 0` 等の plugin option は使わない [S3-006] [S7-003]
...
- [ ] random-init detector を、実在する API に基づいた以下のいずれか (or 複数併用) で実装すること (Stage 3 影響範囲レビュー or 設計フェーズで確定):
   ...
   - (a) **checkpoint integrity hash 検証** — `photon_mlx/checkpoint.py` の `integrity.json` (DR4-003) が存在し、test 内で SHA-256 一致を確認できること。production path (`_load_photon_checkpoint` → `photon_mlx.trainer.load_checkpoint`) は `photon_mlx.checkpoint.load_checkpoint(..., verify_integrity=False)` 相当で、`integrity.json` が存在すれば検証される。本 Issue では production API signature (`_load_photon_checkpoint(model, path)` / `photon_mlx.trainer.load_checkpoint(model, path)`) を変更しない。strict presence まで assert する場合は test 内で `photon_mlx.checkpoint.load_checkpoint(..., verify_integrity=True)` を直接呼ぶに留め、`trainer.load_checkpoint` / `_load_photon_checkpoint` への `verify_integrity` plumbing は follow-up Issue とする。SHA-256 比較は "checkpoint が物理的に存在し改竄されていない" を保証するのみで "weight が model に load された" の verify には (b) との組み合わせが必要 [S3-010] [S5-003] [S7-004]
   ...
   - (c) **PHOTON_ALLOW_RANDOM_INIT bypass のネガティブテスト** — `cfg.model.checkpoint_path` は設定し、`weights.npz` / `state.json` が存在する valid-shaped だが load に失敗する checkpoint (例: corrupt `weights.npz`、integrity mismatch、または state JSON 不正) を使う。env 未設定では RuntimeError、`PHOTON_ALLOW_RANDOM_INIT=1` では WARNING + 続行を assert (`baseline_reporag/photon_pipeline.py:392-413` の挙動を pin する)。`checkpoint_path` 未設定は env と無関係に WARNING のみ (`baseline_reporag/photon_pipeline.py:414-419`) であり、存在しない path / shape 不正は `_resolve_checkpoint_path` / shape validation が try ブロック外なので env=1 でも RuntimeError [S5-001]
...
- [ ] CI 実行戦略を以下の事実認識のもとで確定する (Stage 3 影響範囲レビュー or 設計フェーズで決定):
   ...
   - 採用: weekly_eval.yml に `Run real-weight integration test` step を追加 (PR blocking ではない scheduled/manual workflow で検出する)。毎 PR 用 workflow は現行 main に存在しないため、本 Issue では新規 PR-level workflow は追加しない
   - 運用詳細 (S3-004):
     - 挿入位置: `Install dependencies` step の **後** (長時間 eval の前に 60 秒 budget の load-path regression を fail-fast で検出する)。既存 eval 完走後に置く場合は `if: always()` で必ず実行されるようにする
     - 失敗時挙動: `continue-on-error: true` は使わず、integration test failure で scheduled/manual workflow を fail させる (PR を block しないことと nightly/manual を赤にすることは両立する) [S7-001]
...
- [ ] (推奨 / S1-009 / S5-001) `PHOTON_ALLOW_RANDOM_INIT` の挙動を pin する: `cfg.model.checkpoint_path` に valid-shaped だが load-invalid な checkpoint を設定し、env 未設定 → RuntimeError、env=1 → WARNING + 続行、を両方 assert (`baseline_reporag/photon_pipeline.py:392-413` の挙動を回帰防止)。別途、checkpoint_path 未設定 → env と無関係に WARNING のみ、存在しない path / shape 不正 → env と無関係に RuntimeError、も仕様として混同しない
...
- `tests/integration/conftest.py` (新規, 必要なら。env isolation fixture (`PHOTON_CHECKPOINT_ROOT` / `PHOTON_ALLOW_RANDOM_INIT`), fake tokenizer fixture, MLX availability guard, real `PhotonModel` guard を共通化する候補。単一 test file 内 fixture で済ませる場合は設計フェーズで `conftest.py` 不要と判断する) [S7-002]
...
- `configs/photon_test_minimal.yaml` (新規, test 専用最小 PhotonConfig — 既存 `configs/photon_tiny.yaml` を流用しない理由: tiny も hidden=640 / 80M params で学習に分単位かかるため。candidate B は既存 `photon_mlx/tests/test_training.py::_tiny_cfg` と同型にし、`base_embed_dim: 16`, `hidden_size: 64`, `intermediate_size: 128`, `num_heads: 4`, `head_dim: 16`, `tokenizer.vocab_size: 256`, `hierarchy.chunk_sizes: [4, 4]`, `encoder_layers_per_level: [1, 1]`, `decoder_layers_per_level: [1, 1]` を想定 [S5-004])
...
- `.github/workflows/weekly_eval.yml` (scope 内必須。scheduled/manual workflow に `Run real-weight integration test` step を追加し、`continue-on-error: true` は使わない。運用詳細は受入条件 CI 実行戦略の S3-004 / S7-001 項を参照)
...
- `docs/deployment.md` / `docs/troubleshooting.md` / `docs/code_review_checklist.md` (整合確認。Stage 5 で精密化した `PHOTON_ALLOW_RANDOM_INIT` の適用範囲が docs と矛盾しないか確認し、必要なら「valid-shaped load failure のみ bypass、checkpoint_path 未設定/不在/shape 不正は対象外」を補足する。更新不要と判断する場合も設計フェーズで明記する) [S7-005]
...
- 衝突確認:
  - #138 tokenizer mismatch: 本 Issue の local tokenizer fixture は `vocab_size == expected_vocab_size` に揃え、`_load_hf_tokenizer` invariant と矛盾しないようにする [S5-005]
  - #140 random-init WARNING: `PhotonInference` 起動時の embedding variance WARNING は caplog で明示的に許容または抑制方針を置く。最小 1-step checkpoint は品質評価用の trained checkpoint ではないため、WARNING の有無を品質指標として扱わない [S5-005]
  - #143 eval reproducibility / #156 refusal-aware: 本 Issue は quality eval / citation grading を実施しない load-path integration test であるため直接 scope 外。評価値や refusal-aware 集計を受入条件に混ぜ込まない [S5-005]
...
- Stage 5 通常レビュー (2回目) 反映 (2026-04-28): S5-001 (`PHOTON_ALLOW_RANDOM_INIT` は checkpoint_path 未設定/不在ではなく valid-shaped load failure のみ bypass 可能な点を修正), S5-002 (ゴールを方針 B の checkpoint load path integration に合わせ、実 tokenizer/retrieval full E2E は follow-up と明記), S5-003 (`verify_integrity=True` は `photon_mlx.checkpoint.load_checkpoint` 側の API であり trainer wrapper には未 exposed と明記), S5-004 (最小 config 寸法を既存 `_tiny_cfg` と同型に具体化), S5-005 (#138/#140/#143/#156 との衝突条件を明記)
  - workspace/issues/145/issue-review/stage5-review-result.json
- Stage 7 影響範囲レビュー (2回目) 反映 (2026-04-28): S7-001 (weekly_eval.yml への integration step 追加を scope 内必須化し、`continue-on-error: true` を禁止して scheduled/manual workflow で fail-fast 検出), S7-002 (`tests/integration/conftest.py` 追加要否を影響ファイルに明示), S7-003 (`--forked` / `-n 0` 方針を削除し plugin 依存なしの単独・シリアル実行へ修正), S7-004 (production API signature を本 Issue では変更しない方針を明記), S7-005 (docs/deployment.md / docs/troubleshooting.md / docs/code_review_checklist.md の整合確認を影響ファイルに追加)
  - workspace/issues/145/issue-review/stage7-review-result.json
