...
- auto-skip 廃止は該当セクションだけでなく、`.claude/commands/multi-stage-issue-review.md` 内の summary report テンプレートと完了条件にも反映する。具体的には `5-8 | ... | 完了/スキップ` のような旧表現と「2回目イテレーション自動スキップ」注記を更新し、Stage 5/7 の review-result が常に生成される前提に揃える (S7-002)。
...
**実装形態** (S7-001 反映): 本 Issue の最小スコープでは、PM command Markdown に JSON パース検証 snippet と completion report テンプレートを追記し、`tests/test_skill_descriptions.py` で snippet / テンプレートの存在と fixture 文字列を検証する。実行可能な共通 helper script として実装する場合は、設計フェーズでパスを確定し、影響ファイルと受入条件に helper script と unit test を追加する。
...
**WorkingMemoryConfig 案を採用する場合の追加注意** (S7-003):

- `PhotonInference.__init__` は `working_memory_cfg` が `_UNSET` / `None` / `WorkingMemoryConfig` の 3 状態を取り得るため、それぞれで閾値をどこから読むかを設計で明確化する。
- baseline 経路では `session_memory.working_memory` から `WorkingMemoryConfig` を解決するため、WorkingMemoryConfig に閾値を置く場合は `configs/photon_*.yaml` load 互換テストだけでは不十分。`baseline_reporag` 側の YAML roundtrip test も追加する。
- `configs/photon_*.yaml` load 互換テストは ModelConfig / 新規 RuntimeChecksConfig 経路を選ぶ場合の確認として扱い、WorkingMemoryConfig 経路を選ぶ場合は別テストが必要である。
...
- [ ] **Task 1**: 両 skill (`multi-stage-design-review.md`, `multi-stage-issue-review.md`) の description に「Codex 担当 Stage 必須」明記。`multi-stage-issue-review.md` の Stage 5-8 自動スキップ判定を廃止し、1回目 Must Fix 0 件でも Codex Stage を実行することを明記。summary report テンプレートと完了条件からも旧 auto-skip 前提を除去。test 配置先 = `tests/test_skill_descriptions.py` (新規) で string-existence test 各 1 ケース (S3-005 / S7-002)。
- [ ] **Task 2**: `/pm-auto-issue2dev` / `/pm-auto-design2dev` の completion report テンプレートに Stage 別 finding 数表 (Issue review = Stage 1/3/5/7、Design review = Stage 1-4) が含まれることの test fixture。`reviewer="codex"` 検証用の JSON パース snippet または helper script を通る test 1 件以上 (apply-result.json は対象外)。欠落・`reviewer!="codex"` ケースが WARNING + completion report 記録になることも fixture で確認する (S7-001)。
...
- [ ] **Task 4 config 経路**: 閾値 field の設置先を設計フェーズで確定し、選んだ経路に応じた互換 test を追加する。WorkingMemoryConfig 案を採用する場合は `_UNSET` / `None` / 明示 WorkingMemoryConfig の閾値解決規則と `baseline_reporag` YAML roundtrip test を含める (S7-003)。
...
- (任意) `scripts/validate_review_results.py` 等 (Task 2 の reviewer 検証を Markdown snippet ではなく共通 helper として実装する場合のみ、S7-001)
